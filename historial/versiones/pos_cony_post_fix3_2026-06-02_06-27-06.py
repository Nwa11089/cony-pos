...
def corte_cerrar(data: dict, user: dict = Depends(authenticate)):
    ...
    # Validar que NO haya pedidos pendientes (estado=1 pendiente, estado=0 cancelado, estado=3 en-preparacion)
    # Solo permitir cerrar si todos los pedidos están completados (estado=2)
    pendientes = conn.execute(
        "SELECT COUNT(*) FROM pedidos WHERE corte_id = ? AND estado NOT IN (2, 0)",
        (corte["id"],)
    ).fetchone()[0]
    if pendientes > 0:
        conn.close()
        raise HTTPException(400, f"⚠️ Hay {pendientes} pedido(s) pendiente(s) o en preparación. Deben estar marcados como LISTOS antes de cerrar corte.")
    ...
    # efectivo_reportado = lo que el cajero reporta haber generado en ventas efectivo
    # se calcula como: arqueo_total - fondo_inicial
    if total_arqueado:
