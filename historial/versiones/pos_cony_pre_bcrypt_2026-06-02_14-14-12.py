...
def corte_cerrar(data: dict, user: dict = Depends(authenticate)):
    ...
    # Solo permitir cerrar si todos los pedidos están completados (estado=2) o ya cancelados (estado=0)
    pendientes = conn.execute(
        "SELECT COUNT(*) FROM pedidos WHERE corte_id = ? AND estado NOT IN (2, 0)",
        (corte["id"],)
    ).fetchone()[0]
    # También verificar si hay cancelaciones pendientes de autorizar
