...
def listar_pedidos(user: dict = Depends(authenticate)):
    conn = get_db()
    # Obtener el último corte abierto o cerrado
    cursor = conn.execute("SELECT id, estado FROM cortes ORDER BY id DESC LIMIT 1")
    ultimo_corte = cursor.fetchone()
    if ultimo_corte:
        corte_id = ultimo_corte["id"]
        # Si el último corte está cerrado, mostrar solo pedidos de ese corte
        # Si está abierto, mostrar pedidos del corte abierto
        cursor = conn.execute("""
...
def ventas_hoy(user: dict = Depends(authenticate)):
    ...
    cursor = conn.execute(
        "SELECT COUNT(*) as total_pedidos, COALESCE(SUM(total),0) as total_ventas FROM pedidos WHERE fecha LIKE ? AND estado != 0",
        (hoy+"%",)
    )
...
def actualizar_datos_usuario(uid: int, data: dict, user: dict = Depends(authenticate)):
    ...
    conn.execute(
        ...
        (nombre_real, apellidos, telefono, uid)
    )
    # Actualizar rol si viene en los datos y no es superuser
    rol = data.get("rol", "")
    if rol and rol in ("admin", "user"):
        conn.execute("UPDATE usuarios SET rol=? WHERE id=? AND rol != 'superuser'", (rol, uid))
    conn.commit()
    conn.close()
    return {"status": "ok"}
