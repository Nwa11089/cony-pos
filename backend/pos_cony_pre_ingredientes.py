...
def get_pedidos_menu():
    return HTMLResponse(open("/var/www/pos+ia/clientes/demo/frontend/frontend_demo/pedidos.html").read())

@app.get("/delivery", response_class=HTMLResponse)
def get_delivery_panel():
    return HTMLResponse(open("/var/www/pos+ia/clientes/demo/frontend/frontend_demo/delivery.html").read())
...
# ==================== PEDIDOS PÚBLICOS (menú digital) ====================
class PedidoMenuIn(BaseModel):
    cliente: str
    telefono: str = ""
    direccion: str = ""
    ...
    items: list = []
    total: float = 0
    origen: str = "menu_digital"

@app.post("/api/pedidos/menu")
def crear_pedido_menu(p: PedidoMenuIn):
    """Crea un pedido desde el menú digital público"""
    conn = get_db()
    ...
    cursor = conn.execute("SELECT id FROM clientes WHERE telefono = ?", (p.telefono,))
    cliente = cursor.fetchone()
    if cliente:
        cliente_id = cliente["id"]
        conn.execute("UPDATE clientes SET nombre = ? WHERE id = ?", (p.cliente, cliente_id))
    else:
        conn.execute(
            "INSERT INTO clientes (nombre, telefono, direccion) VALUES (?,?,?)",
            (p.cliente, p.telefono, p.direccion)
        )
        cliente_id = cursor.lastrowid
    
    # Obtener corte activo
    cursor = conn.execute("SELECT id FROM cortes WHERE estado = 'abierto' LIMIT 1")
    ...
    conn.execute(
        "INSERT INTO pedidos (cliente_id, productos, total, metodo_pago, nota, estado, fecha, corte_id, origen) VALUES (?,?,?,?,?,?,?,?,?)",
        (cliente_id, items_json, p.total, p.metodo_pago, notas, 1, fecha, corte_id, p.origen)
    )
    pedido_id = cursor.lastrowid
    conn.commit()
    conn.close()
    return {"id": pedido_id, "status": "creado", "mensaje": f"Pedido #{pedido_id} registrado"}
...
# ==================== VERIFICACIÓN DE LOGIN (para panel delivery) ====================
@app.get("/api/delivery/verify-login")
def verify_delivery_login(user: dict = Depends(authenticate)):
    """Verifica que las credenciales sean válidas para el panel delivery."""
    return {"status": "ok", "user": user.get("nombre", user.get("username", "admin")), "role": user.get("rol", "admin")}

@app.get("/api/delivery/pedidos")
def delivery_pedidos(user: dict = Depends(authenticate)):
    """Devuelve solo pedidos de delivery/menú digital para el panel."""
    conn = get_db()
    cursor = conn.execute(
        "SELECT p.id, p.cliente_id, COALESCE(c.nombre,'Delivery') as cliente_nombre, "
        "p.productos, p.total, p.metodo_pago, p.folio as nota, p.estado, p.fecha, p.origen "
        "FROM pedidos p LEFT JOIN clientes c ON p.cliente_id = c.id "
        "WHERE p.origen LIKE 'delivery_%%' OR p.origen = 'menu_digital' "
...
@app.get("/api/delivery/repartidores")
def delivery_repartidores(user: dict = Depends(authenticate)):
    """Devuelve lista de repartidores activos."""
    conn = get_db()
    cursor = conn.execute("SELECT id, nombre, telefono FROM usuarios WHERE rol = 'repartidor'")
    repartidores = [dict(row) for row in cursor.fetchall()]
    conn.close()
    return {"repartidores": repartidores}

# 📦 INTEGRACIÓN DELIVERY — Rappi / Uber Eats / Didi Food
# Endpoints webhook para recibir pedidos de plataformas externas
# ====================================================================

class DeliveryOrderIn(BaseModel):
    """Estructura unificada de pedido delivery.
    Cada plataforma se mapea a este formato antes de insertar.
    """
    plataforma: str  # "rappi", "ubereats", "didi", "menudigital"
    pedido_externo_id: str = ""
    cliente_nombre: str = "Delivery"
    ...
    envio_costo: float = 0
    notas: str = ""
    folio_plataforma: str = ""
    raw_payload: dict = {}  # payload original por si se necesita


def _insertar_pedido_delivery(p: DeliveryOrderIn) -> dict:
    """Inserta un pedido delivery en la DB. Reutilizable desde cualquier plataforma."""
    conn = get_db()
    from datetime import datetime as _dt
    ahora = _dt.now().strftime("%Y-%m-%d %H:%M:%S")
    
    tel = p.cliente_telefono or "0000000000"
    cur_cliente = conn.execute("SELECT id FROM clientes WHERE telefono = ?", (tel,))
    cliente_row = cur_cliente.fetchone()
    if cliente_row:
        cliente_id = cliente_row["id"]
        conn.execute("UPDATE clientes SET nombre = ? WHERE id = ?", (p.cliente_nombre, cliente_id))
    else:
        conn.execute(
            "INSERT INTO clientes (nombre, telefono) VALUES (?,?)",
            (p.cliente_nombre, tel)
        )
        cliente_id = conn.execute("SELECT last_insert_rowid()").fetchone()[0]
    
    cur_corte = conn.execute("SELECT id FROM cortes WHERE estado = 'abierto' LIMIT 1")
    corte_row = cur_corte.fetchone()
    corte_id = corte_row["id"] if corte_row else 1
    
    import json as _json
    items_json = _json.dumps(p.items, ensure_ascii=False)
    notas = p.notas or ""
    origen = "delivery_" + p.plataforma
    
    conn.execute(
        """INSERT INTO pedidos 
        (cliente_id, productos, total, metodo_pago, metodo_entrega,
         envio_costo, folio, nota, estado, fecha, corte_id, origen)
        VALUES (?, ?, ?, ?, 'Delivery', ?, ?, ?, 1, ?, ?, ?)""",
        (cliente_id, items_json, p.total, p.metodo_pago,
         p.envio_costo, p.folio_plataforma, notas, ahora, corte_id, origen)
    )
    pedido_id = conn.execute("SELECT last_insert_rowid()").fetchone()[0]
    conn.commit()
    conn.close()
    
    return {"id": pedido_id, "plataforma": p.plataforma, "folio": p.folio_plataforma, "status": "creado"}


def _responder_ok(plataforma: str, pedido_id: int):
    """Responde éxito en el formato que espera cada plataforma."""
    if plataforma == "rappi":
        return {"status": "ok", "order_id": pedido_id, "message": "Order received"}
    elif plataforma == "ubereats":
        return {"status": "ACCEPTED", "order_id": pedido_id}
    elif plataforma == "didi":
        return {"code": 0, "data": {"order_id": pedido_id}}
    return {"status": "ok", "pedido_id": pedido_id}


def _mapear_rappi(payload: dict) -> DeliveryOrderIn:
    items = []
    for i in (payload.get("items") or payload.get("products") or []):
        items.append({
            "id_externo": i.get("id") or i.get("product_id"),
            "nombre": i.get("name") or i.get("description", "Sin nombre"),
            "cantidad": int(i.get("quantity", 1)),
            "precio": float(i.get("unit_price", i.get("price", 0))),
            "notas": i.get("options", i.get("notes", "")),
            "total": float(i.get("total", i.get("unit_price", 0) * int(i.get("quantity", 1))))
        })
    delivery = payload.get("delivery", payload.get("shipping", {}))
    cliente = payload.get("customer", payload.get("client", {}))
    return DeliveryOrderIn(
        plataforma="rappi",
        pedido_externo_id=str(payload.get("id") or payload.get("order_id", "")),
        cliente_nombre=cliente.get("name", cliente.get("nombre", "Delivery Rappi")),
        cliente_telefono=cliente.get("phone", cliente.get("telefono", "")),
        metodo_pago=payload.get("payment_method", payload.get("metodo_pago", "Efectivo")),
        items=items,
        total=float(payload.get("total", payload.get("total_amount", 0))),
        envio_costo=float(delivery.get("cost", delivery.get("costo", 0))),
        notas=payload.get("notes", payload.get("notas", "")),
        folio_plataforma="RAPPI-" + str(payload.get('id', payload.get('order_id', ''))),
        raw_payload=payload
    )


def _mapear_ubereats(payload: dict) -> DeliveryOrderIn:
    items = []
    for i in (payload.get("items") or payload.get("products") or []):
        items.append({
            "id_externo": i.get("id") or i.get("item_id"),
            "nombre": i.get("title") or i.get("name", "Sin nombre"),
            "cantidad": int(i.get("quantity", 1)),
            "precio": float(i.get("unit_price", i.get("price", 0))),
            "notas": i.get("special_instructions", i.get("notes", "")),
            "total": float(i.get("total", i.get("unit_price", 0) * int(i.get("quantity", 1))))
        })
    delivery = payload.get("delivery", {})
    cliente = payload.get("customer", {})
    return DeliveryOrderIn(
        plataforma="ubereats",
        pedido_externo_id=str(payload.get("order_id", payload.get("id", ""))),
        cliente_nombre=cliente.get("name", cliente.get("first_name", "Delivery UE")),
        cliente_telefono=cliente.get("phone", ""),
        metodo_pago=payload.get("payment_type", "Tarjeta"),
        items=items,
        total=float(payload.get("total", payload.get("total_amount", 0))),
        envio_costo=float(payload.get("delivery_fee", 0)),
        notas=payload.get("notes", payload.get("instructions", "")),
        folio_plataforma="UBER-" + str(payload.get('order_id', payload.get('id', ''))),
        raw_payload=payload
    )


def _mapear_didi(payload: dict) -> DeliveryOrderIn:
    items = []
    for i in (payload.get("items") or payload.get("dishes") or []):
        items.append({
            "id_externo": i.get("id") or i.get("dish_id"),
            "nombre": i.get("name") or i.get("dish_name", "Sin nombre"),
            "cantidad": int(i.get("quantity", 1)),
            "precio": float(i.get("price", i.get("unit_price", 0))),
            "notas": i.get("remark", i.get("notes", "")),
            "total": float(i.get("total", i.get("price", 0) * int(i.get("quantity", 1))))
        })
    delivery = payload.get("delivery", payload.get("shipping", {}))
    cliente = payload.get("customer", payload.get("user", payload.get("client", {})))
    return DeliveryOrderIn(
        plataforma="didi",
        pedido_externo_id=str(payload.get("order_id", payload.get("id", ""))),
        cliente_nombre=cliente.get("name", cliente.get("nickname", "Delivery Didi")),
        cliente_telefono=cliente.get("phone", cliente.get("mobile", "")),
        metodo_pago=payload.get("payment", payload.get("payment_method", "Efectivo")),
        items=items,
        total=float(payload.get("total", payload.get("order_amount", 0))),
        envio_costo=float(payload.get("delivery_fee", payload.get("shipping_fee", 0))),
        notas=payload.get("remark", payload.get("notes", "")),
        folio_plataforma="DIDI-" + str(payload.get('order_id', payload.get('id', ''))),
        raw_payload=payload
    )
...
@app.post("/api/delivery/rappi")
def webhook_rappi(payload: dict):
    try:
        order = _mapear_rappi(payload)
        result = _insertar_pedido_delivery(order)
        return _responder_ok("rappi", result["id"])
    except Exception as e:
        raise HTTPException(500, detail={"status": "error", "message": str(e)})


@app.post("/api/delivery/ubereats")
def webhook_ubereats(payload: dict):
    try:
        order = _mapear_ubereats(payload)
        result = _insertar_pedido_delivery(order)
        return _responder_ok("ubereats", result["id"])
    except Exception as e:
        raise HTTPException(500, detail={"status": "REJECTED", "reason": str(e)})


@app.post("/api/delivery/didi")
def webhook_didi(payload: dict):
    try:
        order = _mapear_didi(payload)
        result = _insertar_pedido_delivery(order)
        return _responder_ok("didi", result["id"])
    except Exception as e:
        raise HTTPException(500, detail={"code": -1, "message": str(e)})


@app.post("/api/delivery/test")
def webhook_test(payload: dict):
    """Endpoint de prueba para simular un pedido delivery."""
    plat = payload.get("plataforma", "menudigital")
    order = DeliveryOrderIn(
        plataforma=plat,
        cliente_nombre=payload.get("cliente_nombre", payload.get("cliente", "Test")),
        cliente_telefono=payload.get("cliente_telefono", payload.get("telefono", "")),
        metodo_pago=payload.get("metodo_pago", "Efectivo"),
        items=payload.get("items", []),
        total=float(payload.get("total", 0)),
        notas=payload.get("notas", ""),
        folio_plataforma="TEST-" + datetime.now().strftime('%H%M%S'),
        raw_payload=payload
    )
    result = _insertar_pedido_delivery(order)
...
if __name__ == "__main__":
    # Este bloque se ejecuta solo si se corre directo
    import uvicorn
    import argparse
